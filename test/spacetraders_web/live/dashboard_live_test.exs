defmodule SpaceTradersWeb.DashboardLiveTest do
  ...
  describe "signed-in operator" do
    ...
    test "retains stale Agents in the recovery card until replacement succeeds", %{
      conn: conn,
      operator: operator
    } do
      ...
      {:ok, _lv, html} = live(conn, ~p"/")
      assert html =~ "Your stale Agents are no longer available"
      assert html =~ "remain available until a replacement Agent is ready"
      refute html =~ "Retire stale Agents"
