defmodule SpaceTraders.Fleet do
  ...
  if false do
    ...
    def configure_survey_job(%AgentRecord{agent_token: token} = agent, ship_symbol, attrs)
      ...
      with :ok <- legacy_job_admission(agent),
           ...
           {:ok, live_ship} <-
             Agent.handle_game_result(
               agent,
               SpaceTraders.Evidence.get_ship(token_reference(agent), ship_symbol)
             ),
           {:ok, waypoint} <-
        ...
        job =
          ...
          |> Ecto.Changeset.put_change(:progress, %{
            "target_system" => live_ship.nav.system_symbol
          })
          |> Repo.insert()
    ...
    defp reconcile_outfitting_intent(agent, job, intent, live_ship) do
      case Intents.advance(agent, intent, live_ship) do
    ...
    defp authorized_removal(progress, live_ship) do
      Enum.find_value(progress["authorized_removals"], fn {symbol, count} ->
        ...
        if count > removed and Intents.module_count(live_ship.modules, symbol) > 0, do: symbol
      end)
    end
    ...
    defp apply_unapplied_outfitting_purchase!(job) do
      intent = Intents.last_completed_job_intent(job.id, "buy")
    ...
    defp selectable_market_reconnaissance_job(agent_id, job_id) do
      Repo.one(
        from job in Job,
          ...
          where:
            job.id == ^job_id and ship.agent_id == ^agent_id and
              job.type == "market_reconnaissance" and
    ...
    defp advance_market_reconnaissance_job(agent, %Job{} = job, live_ship) do
      ...
      case Enum.find(stops, &(&1 not in completed)) do
        ...
        stop
        when live_ship.nav.status != "IN_TRANSIT" and live_ship.nav.waypoint_symbol == stop ->
    ...
    def start_procurement_job(%AgentRecord{} = agent, ship_symbol) do
      with :ok <- legacy_job_admission(agent),
           ...
           nil <- Intents.unfinished_manual_intent(ship.id),
           nil <- Intents.unresolved_cargo_intent(ship.id) do
        job = apply_unapplied_procurement_intent!(job)

        case Intents.unfinished_job_intent(job.id) do
          %Intent{} = intent ->
            job =
              Repo.update!(
                Ecto.Changeset.change(job, status: "active", blocker: nil, blocked_reason: nil)
    ...
    defp reconcile_procurement_intent(agent, ship, job, intent) do
      with {:ok, live_ship} <-
             Agent.handle_game_result(
               agent,
               SpaceTraders.Evidence.get_ship(token_reference(agent), ship.symbol)
             ),
           :ok <- procurement_system_matches?(job.progress, live_ship),
    ...
    defp pause_job_type(agent, ship_symbol, type, label) do
      with {:ok, ship} <- owned_ship(agent, ship_symbol),
        ...
        case Repo.transaction(
               fn ->
                 Intents.terminalize_job_intent!(job)
    ...
    defp initialize_procurement_progress(job, live_ship, recipient) do
      ...
      cond do
        not progress["compatible_existing_cargo"] and
          not is_integer(progress["accepted_baseline"]) and
            held > 0 ->
    ...
    defp start_procurement_intent(agent, job, live_ship, recipient, credits) do
      ...
      with {:ok, attrs} <-
             procurement_intent_attrs(agent, live_ship, progress, recipient, credits),
    ...
    defp procurement_intent_request(agent, job, live_ship, attrs) do
      with {:ok, intent} <- Intents.insert_job_intent(job, attrs) do
    ...
    defp procurement_intent_attrs(agent, live_ship, progress, recipient, credits) do
      units =
        min(progress["aboard"], max(progress["requested"] - progress["shared_fulfilled"], 0))
      ...
      cond do
        ...
        true ->
          with {:ok, source} <- procurement_source(agent, live_ship, progress),
               ...
               {:ok, units} <-
                 procurement_purchase_units(live_ship, source.good, progress, credits) do
            {:ok,
             %{
               type: "buy",
               target_waypoint: source.waypoint,
    ...
    defp realized_market_trade_progress(progress, buy, sell) do
      with %{"total_price" => purchase_total} <- get_in(buy.last_action_result, ["transaction"]),
        ...
        estimated_fuel_cost =
          get_in(sell.parameters, ["market_trade", "estimated_fuel_cost"]) || 0
          ...
          case intent do
            ...
            %Intent{type: "deliver", last_action_result: result} ->
              if is_integer(result["units"]),
                do:
                  Map.update(job.progress, "accepted", result["units"], &(&1 + result["units"])),
                else: job.progress
    ...
    defp apply_unapplied_procurement_intent!(job) do
      intent = Intents.last_completed_job_intent(job.id)
    ...
    defp block_procurement_cargo_intent(intent, reason) do
      case Intents.transition_intent(intent,
             status: "blocked",
             blocker: job_blocker(reason),
             in_flight_action:
               if(Intents.ambiguous_cargo_operation_error?(reason),
                 do: intent.in_flight_action,
                 else: nil
               ),
             last_action_result: %{
               "kind" => intent.type,
               "error" => Intents.cargo_error_message(reason)
             }
           ) do
        {:ok, intent} -> intent
        :intent_no_longer_owned -> :ok
    ...
    def contract_delivery_evidence(contract, trade_symbol) do
      case find_deliverable(contract, trade_symbol) do
        ...
        delivery ->
          %{"trade_symbol" => trade_symbol, "units_fulfilled" => delivery.units_fulfilled}
      end
    end
    ...
    defp procurement_purchase_units(live_ship, good, progress, credits) do
      affordable =
        Intents.affordable_cargo_units(
    ...
    defp construction_supply_counts(progress, live_ship, construction) do
      ...
      remaining =
        Map.new(requirements, fn requirement ->
          {requirement["trade_symbol"],
           max(requirement["required"] - requirement["fulfilled"], 0)}
        end)
    ...
    defp start_construction_supply_intent(agent, job, live_ship, construction, credits) do
      with {:ok, attrs} <-
             construction_supply_intent_attrs(
               agent,
               job.progress,
               live_ship,
               construction,
    ...
    defp construction_supply_purchase_units(live_ship, good, needed, progress, credits) do
      ...
      units =
        min(
          ...
          min(needed, Intents.affordable_cargo_units(total_budget, good.purchase_price))
        )
    ...
    defp scan_explorer_waypoints(agent, credential_ref, job, live_ship) do
      cond do
        ...
        true ->
          ...
          job =
            Repo.update!(Ecto.Changeset.change(job, status: "active", in_flight_action: action))
      ...
      Repo.transaction(
        fn ->
          ...
          cond do
            ...
            match?(
              %Intent{in_flight_action: action} when is_map(action),
              Intents.unfinished_job_intent(current.id)
            ) ->
              Repo.rollback(:job_action_reconciliation_required)
    ...
    defp record_command_result(agent, ship_symbol, kind, {:ok, _result} = result) do
      record_command_activity(
        ...
        ship_symbol,
        kind,
        "#{command_label(kind)} command completed"
      )
    ...
    defp perform_survey_action(agent, job, live_ship) do
      ...
      with {:ok, result} <-
           ...
           :ok <-
             persist_surveys(agent, result.surveys, job.extraction_waypoint, live_ship.symbol),
    ...
    defp arrived_at_configured_waypoint?(
      ...
      cond do
        live_ship.nav.status == "DOCKED" ->
          case Agent.handle_game_result(
            ...
            {:ok, result} ->
              advance_miner_job(agent, config, %{live_ship | nav: result.nav}, mode)
    ...
    defp configured_sellable_goods(%Job{sellable_goods: goods})
         when is_list(goods) and goods != [],
    ...
    @doc false
    def record_construction_observation(
    ...
    defp record_jump_gate_observation(%AgentRecord{id: id} = agent, system, gate, source)
      ...
      exception ->
        Logger.warning(
          "Could not persist jump-gate intelligence: #{Exception.message(exception)}"
        )
    end
        ...
        Repo.transaction(fn ->
          ...
          if existing do
            ...
            Repo.update_all(
              from(destination in ShipDestination,
                where:
                  destination.ship_id == ^ship.id and destination.position > ^existing.position
              ),
              inc: [position: -1]
            )
          end
    ...
    defp invalidate_market_facts(agent, system_symbol, waypoint_symbol) do
      ...
      exception ->
        Logger.warning(
          "Could not invalidate market intelligence: #{Exception.message(exception)}"
        )
    end
    ...
    def transfer_cargo(
      ...
      with :ok <- Agent.execution_allowed?(agent),
           ...
           {:ok, source} <-
             Agent.handle_game_result(
               agent,
               SpaceTraders.Evidence.get_ship(token_reference(agent), from_ship)
             ),
           {:ok, target} <-
             Agent.handle_game_result(
               agent,
               SpaceTraders.Evidence.get_ship(token_reference(agent), to_ship)
             ),
        ...
        if config.type in [
          ...
          if config.status in @running_job_states do
            ...
            case Agent.handle_game_result(
              ...
              {:ok, live_ship}
              when config.status == "active" and is_nil(config.in_flight_action) ->
                case config.type do
                  "explorer" ->
                    advance_explorer_job(agent, config, live_ship)
              ...
              when config.status in ["active", "waiting"] and is_map(config.in_flight_action) ->
                case Intents.unfinished_job_intent(config.id) do
    ...
    def reconcile_miner_job(%AgentRecord{} = agent, ship_symbol) do
      with :ok <- legacy_job_admission(agent),
           ...
           {:ok, live_ship} <-
             Agent.handle_game_result(
               agent,
               SpaceTraders.Evidence.get_ship(token_reference(agent), ship_symbol, retry: false)
             ) do
    ...
    defp action_outcome(%{"kind" => "navigate", "waypoint" => waypoint}, live_ship) do
      cond do
        live_ship.nav.status == "IN_TRANSIT" and
            live_ship.nav.route.destination.symbol == waypoint ->
          :confirmed
    ...
    defp retry_recovery(agent_id, ship, config, live_ship) do
      ...
      case result do
        {:ok, recovered_config} ->
          {:ok,
           Repo.update!(
             Ecto.Changeset.change(recovered_config,
               recovery_attempts: 0,
               recovery_started_at: nil
             )
           )}
