    ...
    with {:ok, agent} <- agent_for_ship(socket, ship_symbol),
         ...
         {:ok, %{status: "completed"} = intent} <-
           Intents.request(
             ...
             %Intents.ManualControl{},
             ship_symbol,
             %Intents.BuyGoods{market: waypoint, trade_good: trade_symbol, quantity: units}
           ) do
      socket =
        socket
        |> refresh_agent(agent)
  ...
  defp stale_agent_card(assigns) do
    ...
    <section :if={@stale_agents != []} class="rounded-2xl border border-warning/40 bg-warning/10 p-5">
      ...
      <p class="mt-2 text-sm leading-6">
        The game reset and invalidated {Enum.map_join(@stale_agents, ", ", & &1.symbol)}. These records remain available until a replacement Agent is ready.
      </p>
